...
function result() {
    ...
    let page = 0;
    // let maxLength = 100;
    let maxpage = Math.floor(text.length / 100) + (text.length % 100 == 0 ? 0 : 1);
    for (let i = 0; i < text.length; i += 100) {
        let yolo = text.substr(i, 100)
        yolo += `- ${++page} of ${maxpage}`

        // statt page kann ich auch i / 100 + 1, im Bezug auf i von Zeile 22 da es i += 100 ist
        //yolo += `- ${++page} of 27`, nur wenn du sicher die Seitenanzahl weißt, außer jemand ändert den Text
        parts.push(yolo)
    }
    console.log(parts)
}

//push funktioniert nur mit einem Array
//i < text.length heißt das i kleiner ist als die Textlänge
//i += 100 ist der Zuweisungoperator, ist das gleiche i = i + 100 
// es fängt bei i = an und hört bei 100 auf und dann geht es bis 200, 300, 400...
//substr nimmt den startindex und dann den nächsten x-zeichen
